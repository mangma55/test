...
3. เปิด **Project Settings → API Keys** แล้วคัดลอกเฉพาะ **Project URL** และ **Publishable key** (`sb_publishable_...`) ลง `js/config.js` ถ้าโปรเจกต์เก่าใช้ `anon` key ก็ใช้ได้
4. อัปโหลด `index.html`, `dashboard.html`, `css/` และ `js/` ไปยัง root ของ GitHub repository โดยรักษาโครงสร้างโฟลเดอร์เดิม ไม่จำเป็นต้องเผยแพร่ `setup.sql` หรือ `README.md` แต่เผยแพร่ได้เพราะไม่มี secret
...
- `js/config.js` อยู่ในเว็บสาธารณะ ใช้ได้เฉพาะ publishable/anon key เท่านั้น **ห้าม** ใส่ `sb_secret_...`, `service_role`, รหัสผ่าน หรือโทเค็นผู้ใช้
